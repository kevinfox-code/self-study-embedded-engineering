...
- **ADXL345 driver** (`Src/adx1345.c`, `Inc/adx1345.h`): `adxl_init()` configures the sensor for ±4g range and enables measurement mode; `adxl_read()` performs a multi-byte SPI read of the six acceleration data registers (0x32–0x37) in a single CS-asserted transaction
...
Used Claude (claude-sonnet-4-6) via Claude Code to assist with register-level SPI configuration details, TSIZE/CSTART sequencing, and debugging the CS glitch on init.
